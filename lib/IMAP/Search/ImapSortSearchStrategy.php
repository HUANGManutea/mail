...
class ImapSortSearchStrategy implements ISearchStrategy {
	...
	public function getIds(int $maxResults, array $flags = []): Horde_Imap_Client_Ids {
		if ($this->filter instanceof Horde_Imap_Client_Search_Query) {
			$query = $this->filter;
		} else {
			$searchHelper = new SearchHelper();
			$query = $searchHelper->parseFilterString($this->filter);
		}

		if ($this->cursor !== null) {
			$query->dateTimeSearch(DateTime::createFromFormat("U", $this->cursor), Horde_Imap_Client_Search_Query::DATE_BEFORE);
		}
